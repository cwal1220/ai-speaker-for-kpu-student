...
class Recorder:
	def __init__(self):
		self.THRESHOLD = 1000#500
		self.FORMAT = pyaudio.paInt16
		self.CHANNELS = 1
		self.RATE = 44100
		self.CHUNK = 8192 # int(RATE / 10)
		self.MINIMUM_RECORD_SECONDS = 1 # seconds (approximate)
		self.f = None
	...
	def record_audio(self):
		def is_silent(data):
			...
			self.THRESHOLD = self.THRESHOLD + 0.2 * (max(data) - self.THRESHOLD)
		...
		self.f = open(".sound.raw", "wb+")
		frames.tofile(self.f)
		self.f.write('\0'.encode())
		self.f.seek(0)
		return self.f
